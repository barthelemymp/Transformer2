...
for i in ilist:
    ...
    #whyyy 'cpu?'
    wandb.init(project="Transformer Fullatt large Fam", entity="barthelemymp")
    config_dict = {
    ...
    for epoch in range(num_epochs+1):
        ...
        for batch_idx, batch in enumerate(train_iterator):
            ...
            firstoutput, output = model(inp_data, target[:-1, :])
            accuracyTrain += accuracy(batch, output, onehot=False).item()
            output = output.reshape(-1, output.shape[2])#keep last dimension
            firstoutput = firstoutput.reshape(-1, firstoutput.shape[2])
            if onehot:
            ...
            optimizer.zero_grad()
            firstloss  = criterion(firstoutput, targets_Original)
            loss = criterion(output, targets_Original)
            lossesCE.append(loss.item())
            lossback = loss + firstloss
            lossback.backward()
        ...
        if epoch%1==0:
            with  torch.no_grad():
                for batch_idx, batch in enumerate(val_iterator):
                    ...
                    _,output = model(inp_data, target[:-1, :])
        ...
        if epoch%200==0:
            ...
            model.eval()
            criterionE = nn.CrossEntropyLoss(ignore_index=pds_train.SymbolMap["<pad>"], reduction='none')
            scoreHungarianVal = HungarianMatchingBSfa(pds_val, model,100)
