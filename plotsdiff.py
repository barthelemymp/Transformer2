...
for i in ilist:# range(1000,1500):
    ...
    if os.path.isfile(modelpath):
        ...
        ax1.set_ylabel("$\mathcal{PP}$ - Perplexity", fontsize=37)
        # plt.title("Perplexity at Different Distance-  from Training Set for"+famname, fontsize=25)
        ax1.set_xlabel("Hamming Distance from Training Set", fontsize=37)
        ax2.set_ylabel("$\mathcal{A}$ - Accuracy", fontsize=37)
        plt.title("$\mathcal{PP}$ & $\mathcal{A}$ Distribution "+famname, fontsize=37)

        
        
        #plt.scatter(x,y3, alpha=0.3, color="green", label="Reyni")
        ax1.tick_params(axis='both', labelsize=37)
        ax2.tick_params(axis='both', labelsize=37)
        ax1.legend(lns, labs,fontsize=33, loc=(0.01,0.2))
