...
def difmin(matrix):
    bs = matrix.shape[0]
    modifiedmatrix = np.zeros((bs,bs))
    for i in range(bs):
        modifiedmatrix[i,:] = matrix[i,:] - np.min(matrix[i,:])
            ...
            if abs(outputsize - inputsize)< 0.2*inputsize:
                ...
                for epoch in range(num_epochs+1):
                    ...
                    if epoch%500==0:
                        ...
                        scoHDif= scipy.optimize.linear_sum_assignment(difmin(scoreHungarian))
                        scoreMatchingexp = sum(scoHDif[0]==scoHDif[1])
                        wandb.log({"scoreMatching Val": scoreMatching, "scoreMatching recentered Val": scoreMatchingexp, "epoch":epoch})
