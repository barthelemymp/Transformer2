...
for i in range(20,300):
    ...
    if abs(outputsize - inputsize)< 0.2*inputsize:
        ...
        #whyyy 'cpu?'
        wandb.init(project="Matching check evolution", entity="barthelemymp")
        config_dict = {
          "num_layers": 6,
          "forward_expansion": 4096,
...
# =============================================================================
                    ...
                    step_ev +=1
            wandb.log({"Train loss CE": mean_lossCE,  "Val loss CE": mean_lossCEVal, "epoch":epoch})#,"Val Loss Matching":mean_lossMatchingVal, "alpha":alpha "Train loss Matching": mean_lossMatchingTrain,
            ...
            if epoch%100==0:
                val_iterator_matching = DataLoader(pds_val, batch_size=1,
                                shuffle=True, num_workers=0, collate_fn=default_collate)
                criterionE = nn.CrossEntropyLoss(ignore_index=pds_train.SymbolMap["<pad>"])
                listin = []
                listout = []
                for batch_idx, batch in enumerate(val_iterator_matching):
                    inp_data, target= batch[0], batch[1]
                    listin.append(inp_data)
                    listout.append(target)
                scoreHungarian = np.zeros((len(listin), len(listin)))
                with torch.no_grad():
                    for j in range(len(listin)):
                        ...
                        for i in range(len(listout)):
                            inp = listin[i]
                            output = model(inp, target[:-1, :])
                            output = output.reshape(-1, output.shape[2])
                            loss_eval = criterionE(output, targets_Original).cpu().numpy()
                            scoreHungarian[i,j] = loss_eval
                ...
                wandb.init(project="Final Loop simple", entity="barthelemymp", resume=True)
...
# for i in [17,46,69,71,157,160,251,258]:
#     nameH = "scoH_ARDCA_" +str(i)+ ".npy"
#     scoreHungarian = np.load(nameH)
#     scoH = scipy.optimize.linear_sum_assignment(scoreHungarian)
#     scoreMatching = sum(scoH[0]==scoH[1])
#     print(i, scoreMatching)